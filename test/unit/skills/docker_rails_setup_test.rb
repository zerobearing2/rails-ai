...
class DockerRailsSetupTest < SkillTestCase
  self.skill_domain = "config"
  self.skill_name = "docker-rails-setup"

  def test_skill_file_exists
    assert skill_file_exists?("config", "docker-rails-setup"),
           "docker-rails-setup.md should exist in skills/config/"
  end
  ...
  def test_has_required_metadata
    ...
    assert_equal "docker-rails-setup", skill_metadata["name"]
