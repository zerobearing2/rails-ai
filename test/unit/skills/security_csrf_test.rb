...
class SecurityCsrfTest < SkillTestCase
  ...
  self.skill_name = "security-csrf"

  def test_skill_file_exists
    assert skill_file_exists?("security", "security-csrf"),
           "security-csrf.md should exist in skills/security/"
  end
  ...
  def test_has_required_metadata
    ...
    assert_equal "security-csrf", skill_metadata["name"]
