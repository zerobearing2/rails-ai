...
class RubocopSetupTest < SkillTestCase
  self.skill_domain = "config"
  self.skill_name = "rubocop-setup"

  def test_skill_file_exists
    assert skill_file_exists?("config", "rubocop-setup"),
           "rubocop-setup.md should exist in skills/config/"
  end
  ...
  def test_has_required_metadata
    ...
    assert_equal "rubocop-setup", skill_metadata["name"]
