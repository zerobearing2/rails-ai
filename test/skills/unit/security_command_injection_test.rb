...
class SecurityCommandInjectionTest < SkillTestCase
  self.skill_domain = "security"
  self.skill_name = "security-command-injection"

  def test_skill_file_exists
    assert skill_file_exists?("security", "security-command-injection"),
           "security-command-injection.md should exist in skills/security/"
  end
  ...
  def test_has_required_metadata
    ...
    assert_equal "security-command-injection", skill_metadata["name"]
