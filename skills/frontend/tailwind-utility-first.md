...
class AlertComponent < ViewComponent::Base
  ...
  def alert_classes
    ...
    "#{base} #{variants.fetch(variant, variants[:info])}"
  end
end
```
...
**Usage:** `<%= render AlertComponent.new(message: "Success!", variant: :success) %>`
</pattern>
...
def button_classes(variant, disabled)
  classes = ["px-4 py-2 rounded"]
  classes << "bg-blue-600 text-white" if variant == :primary
...
```erb
<%# Simple conditional in ERB %>
<div class="p-4 <%= "bg-blue-500" if @selected %>">Content</div>

<%# Using helper method %>
<button class="<%= button_classes(:primary, false) %>">Submit</button>
```
</pattern>
...
**Usage:** `<div class="bg-brand text-white">Brand color</div>`
</pattern>
...
<div class="min-h-screen flex flex-col bg-gray-50">
  <header class="bg-white border-b border-gray-200 sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        <%= link_to "App Name", root_path, class: "text-xl font-bold text-gray-900" %>

        <nav class="hidden md:flex items-center gap-6">
          <%= link_to "Dashboard", dashboard_path, class: "text-gray-700 hover:text-gray-900" %>
          <%= link_to "Feedback", feedbacks_path, class: "text-gray-700 hover:text-gray-900" %>
        </nav>
  ...
  <footer class="bg-white border-t mt-auto py-6">
    <p class="text-center text-sm text-gray-500">&copy; <%= Time.current.year %> App</p>
  </footer>
</div>
```
</pattern>
