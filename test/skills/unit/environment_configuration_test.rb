...
class EnvironmentConfigurationTest < SkillTestCase
  self.skill_domain = "config"
  self.skill_name = "environment-configuration"

  def test_skill_file_exists
    assert skill_file_exists?("config", "environment-configuration"),
           "environment-configuration.md should exist in skills/config/"
  end
  ...
  def test_has_required_metadata
    ...
    assert_equal "environment-configuration", skill_metadata["name"]
