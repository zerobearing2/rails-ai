...
class NestedResourcesTest < SkillTestCase
  self.skill_domain = "backend"
  self.skill_name = "nested-resources"

  def test_skill_file_exists
    assert skill_file_exists?("backend", "nested-resources"),
           "nested-resources.md should exist in skills/backend/"
  end
  ...
  def test_has_required_metadata
    ...
    assert_equal "nested-resources", skill_metadata["name"]
