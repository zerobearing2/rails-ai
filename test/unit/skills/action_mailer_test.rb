...
class ActionMailerTest < SkillTestCase
  self.skill_domain = "backend"
  self.skill_name = "action-mailer"

  def test_skill_file_exists
    assert skill_file_exists?("backend", "action-mailer"),
           "action-mailer.md should exist in skills/backend/"
  end
  ...
  def test_has_required_metadata
    ...
    assert_equal "action-mailer", skill_metadata["name"]
